...
- Added the `--dropdown-toc` option to `rimuc` command. This option
  ...
  to the `--toc` sidebar table of contents option. The [home
  page]({homepage}) and [Playground]({playground}) pages are generated
  using the drop-down menu, the [Reference]({reference}) and
  [Tips]({tips}) pages have a sidebar table of contents.
-  The table of contents document title link (generated by the `rimuc`
   `--toc` and `--dropdown-toc` options) navigates to the top of the
   document instead of the title header.
